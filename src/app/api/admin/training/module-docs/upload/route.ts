...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!allowed.includes(ext)) return NextResponse.json({ error: "Only PDF, DOC or DOCX files allowed" }, { status: 400 });
    ...
    const doc = await adminStore.addModuleDoc({ moduleId, title, url, type });
