...
export async function POST(req: NextRequest) {
  ...
  const ch = await adminStore.getChannelById(body.channelId);
  if (!ch || !ch.isActive) {
    return NextResponse.json({ error: "Channel not found or inactive" }, { status: 404 });
  }

  const existing = await adminStore.getUserByEmail(email);
  if (existing && await adminStore.verifyPassword(email, body.password)) {
