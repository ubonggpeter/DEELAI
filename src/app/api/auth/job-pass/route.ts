...
// Called after Paystack payment succeeds to record the job pass
export async function POST(req: NextRequest) {
  ...
  const user = adminStore.updateJobPass(session.userId, permitType, channel.jobPassFee, session.channelId, paystackRef);
  if (!user) return NextResponse.json({ error: "User not found" }, { status: 404 });

  // Refresh cookie with updated session
