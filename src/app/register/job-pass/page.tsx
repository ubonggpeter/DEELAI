...
function JobPassPageInner() {
  ...
  async function handleBuyJobPass() {
    ...
    setPaying(true);
    setTimeout(() => {
      if (!window.PaystackPop) { setPaying(false); void recordJobPass(false, ""); return; }
      const handler = window.PaystackPop.setup({
        key:      channel.paystackPublicKey,
        email:    session.email ?? session.name ?? "user@deelai.uk",
  ...
  return (
    <PageShell step={2} channelName={channel.channelName}>
      ...
      >
        {loading || paying
          ? <><Loader2 size={16} className="spin-icon" /> {paying ? "Opening payment…" : "Recording…"}</>
          : channel.jobPassFee > 0
