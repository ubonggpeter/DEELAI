...
import {
  ...
  Edit2, Check, X, Camera, Loader2, ChevronDown, Building2, CheckCircle2,
  AlertCircle, Save,
} from "lucide-react";
...
// Paystack-supported countries (lowercase match from COUNTRIES list)
const PAYSTACK_SUPPORTED = new Set([
  "nigeria","ghana","kenya","south africa","côte d'ivoire","ivory coast",
  "rwanda","mozambique","cameroon","senegal","egypt","morocco","algeria","tunisia",
  "zambia","zimbabwe","angola","mali","tanzania","uganda",
]);
...
/** Compress + resize an image File client-side using a canvas, returns a new File. */
async function compressImage(file: File, maxPx = 512, quality = 0.82): Promise<File> {
...
export default function ProfileScreen({ user, onBack, setUser }: Props) {
  ...
  const [draftAvatar,  setDraftAvatar]  = useState<string | null>(null); // local preview URL
  const [pendingFile,  setPendingFile]  = useState<File | null>(null);   // compressed file to upload
  ...
  const [bankUnsupported,  setBankUnsupported]  = useState(false);
  ...
  // Track whether profile has unsaved changes
  const nameChanged    = draftName.trim()    !== user.name;
  const countryChanged = draftCountry        !== (user.country ?? "");
  const avatarChanged  = pendingFile !== null;
  const hasChanges     = nameChanged || countryChanged || avatarChanged;

  // Country for bank lookup (draft takes priority)
  const bankCountry = draftCountry || user.country || "";
  const isBankCountrySupported = PAYSTACK_SUPPORTED.has(bankCountry.toLowerCase().trim());

  // Fetch bank list when bank setup opens or when country changes
  useEffect(() => {
    if (!showBankSetup) return;
    setBanks([]); setBankUnsupported(false); setBanksLoading(true);
    const qs = bankCountry ? `?country=${encodeURIComponent(bankCountry)}` : "";
    fetch(`/api/auth/banks${qs}`)
      .then((r) => r.json())
      .then((d) => { setBanks(d.banks ?? []); setBankUnsupported(d.unsupported ?? false); })
      .catch(() => setBankUnsupported(true))
      .finally(() => setBanksLoading(false));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showBankSetup, bankCountry]);

  // Auto-resolve account name when bank + 10-digit number are filled
  useEffect(() => {
  ...
  async function pickAvatar(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setDraftAvatar(URL.createObjectURL(compressed));
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (pendingFile) {
        ...
        const d = await res.json();
        if (!res.ok) { setSaveErr(d.error ?? "Avatar upload failed"); return; }
        newAvatarUrl = d.url;
      }

      // 2. Save name / country / avatarUrl via profile PATCH
      const fields: Record<string, string> = {};
      if (nameChanged)    fields.name     = draftName.trim();
      if (countryChanged) fields.country  = draftCountry;
      if (newAvatarUrl)   fields.avatarUrl = newAvatarUrl;
      ...
      if (setUser) {
        setUser((u) => ({
          ...u,
          name:      nameChanged  ? draftName.trim() : u.name,
          country:   countryChanged ? draftCountry   : u.country,
          avatarUrl: newAvatarUrl ?? u.avatarUrl,
        }));
      }

      // 4. Clear pending changes
      if (draftAvatar) URL.revokeObjectURL(draftAvatar);
      setDraftAvatar(null); setPendingFile(null);
  ...
  async function saveBank() {
    if (!bankName || !acctNum || !acctName || bankSaving) return;
    setBankSaving(true);
  ...
  const displayAvatarUrl = draftAvatar ?? user.avatarUrl;
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-8 text-center relative">
          ...
          <div className="relative inline-block mb-3">
            <div className="flex items-center justify-center rounded-3xl w-20 h-20 sm:w-24 sm:h-24 overflow-hidden"
              ...
              {displayAvatarUrl
                ? <Image src={displayAvatarUrl} alt="Avatar" width={96} height={96} className="w-full h-full object-cover" unoptimized />
            ...
            <button
              ...
              {compressing
                ? <Loader2 size={12} color="#000" style={{ animation:"spin 1s linear infinite" }} />
                : <Camera size={12} color="#000" />}
            </button>
            <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="hidden" onChange={pickAvatar} />
          </div>
          {avatarChanged && (
            <div className="text-[10px] font-mono mb-1" style={{ color: "var(--cyan)" }}>New picture ready — tap Save</div>
          )}
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8, marginBottom:4 }}>
            <input
              ...
              className="text-2xl sm:text-3xl font-bold text-white text-center bg-transparent border-none outline-none"
              style={{ maxWidth:220, borderBottom: nameChanged ? "1px solid rgba(0,212,255,.5)" : "1px solid transparent" }}
            />
            {nameChanged && <Edit2 size={13} style={{ color:"var(--cyan)", flexShrink:0 }} />}
          </div>
          ...
          {hasChanges && (
            <div className="flex items-center justify-center gap-3 mt-5">
              <button
                ...
                style={{ background:"rgba(255,77,109,.12)", color:"#FF4D6D", border:"1px solid rgba(255,77,109,.3)" }}>
                <X size={12} /> Discard
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex items-center gap-1.5 rounded-xl px-5 py-2 text-xs font-bold"
                style={{ background:saving ? "var(--s3)" : "var(--cyan)", color:saving ? "var(--txt3)" : "#000", border:"none", cursor:saving ? "default" : "pointer" }}>
                {saving
          ...
          {savedOk && !hasChanges && (
            <div className="flex items-center justify-center gap-1.5 mt-3 text-xs" style={{ color:"#00E5A0" }}>
              <Check size={13} /> Profile saved
            </div>
          )}
          {saveErr && (
            <div className="flex items-center justify-center gap-1.5 mt-2 text-xs" style={{ color:"#FF4D6D" }}>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
          ...
          <div className="rounded-2xl overflow-hidden mb-5 lg:mb-0"
            ...
            <div className="flex items-center gap-3 p-4 sm:p-5" style={{ borderBottom:"1px solid var(--b1)" }}>
              ...
              <div className="relative flex items-center gap-1">
                {countryChanged && <span className="w-1.5 h-1.5 rounded-full" style={{ background:"var(--cyan)", flexShrink:0 }} />}
                <div className="relative">
                  <select
                    ...
                    style={{ color: draftCountry ? "#fff" : "var(--txt3)" }}>
            ...
            {[
              { Icon: Calendar,   label: "Member Since", value: "January 2026",  action: undefined as (() => void) | undefined },
              { Icon: Share2,     label: "Recruit Code", value: user.refCode,    action: undefined as (() => void) | undefined },
              ...
              <div
                ...
                style={{ borderBottom: i < 2 ? "1px solid var(--b1)" : "none" }}
                onClick={action}
              >
                <Icon size={16} style={{ color:"var(--txt3)", flexShrink:0 }} />
                <span className="text-sm flex-1" style={{ color:"var(--txt2)" }}>{label}</span>
                <span className="text-sm font-medium truncate max-w-[140px]" style={{ color: action ? "var(--cyan)" : "white" }}>
                  {value}
        ...
        {showBankSetup && (
          <div className="mt-5 rounded-2xl p-5" style={{ background:"var(--s1)", border:"1px solid var(--b1)" }}>
            ...
            {!bankCountry && (
              <div className="flex items-center gap-2 mb-4 text-sm rounded-xl p-3"
                style={{ background:"rgba(255,184,0,.08)", border:"1px solid rgba(255,184,0,.2)", color:"#FFB800" }}>
                <AlertCircle size={14} style={{ flexShrink:0 }} />
                Select your region above first so we can show the right banks.
              </div>
            )}

            {bankCountry && !isBankCountrySupported && (
              <div className="flex items-start gap-2 mb-4 rounded-xl p-3 text-sm"
                style={{ background:"rgba(255,184,0,.08)", border:"1px solid rgba(255,184,0,.2)", color:"#FFB800" }}>
                <AlertCircle size={14} style={{ flexShrink:0, marginTop:2 }} />
                <span>Bank auto-lookup via Paystack isn&apos;t available for <strong>{bankCountry}</strong> yet. Enter your details manually.</span>
              </div>
            )}

            {/* Bank selector (dropdown for supported countries, text for others) */}
            <div className="mb-3">
              <label className="text-[10px] font-mono tracking-wider mb-1.5 block" style={{ color:"var(--txt3)" }}>
                {isBankCountrySupported ? "SELECT BANK" : "BANK NAME"}
              </label>
              {banksLoading ? (
              ...
              ) : isBankCountrySupported && banks.length > 0 ? (
                <select
                  value={bankCode}
                  onChange={(e) => {
                  ...
                  className={fs} style={{ cursor:"pointer" }}>
                ...
                <input
                  ...
                  onChange={(e) => { setBankName(e.target.value); setBankCode("manual"); }}
            ...
            <div className="mb-3">
              ...
              <input
                ...
                value={acctNum}
                onChange={(e) => { setAcctNum(e.target.value.replace(/\D/g, "")); setAcctName(""); setResolveErr(""); }}
                placeholder="10-digit account number"
                className={fs}
              />
            </div>

            {/* Auto-resolved account name (Paystack-supported countries) */}
            ...
            {/* Manual account name for unsupported regions */}
            {!isBankCountrySupported && (
              <div className="mb-4">
                <label className="text-[10px] font-mono tracking-wider mb-1.5 block" style={{ color:"var(--txt3)" }}>ACCOUNT HOLDER NAME</label>
                <input
                  type="text" value={acctName}
            ...
            <button
              onClick={saveBank}
              disabled={!bankName || !acctNum || !acctName || bankSaving}
              className="w-full rounded-xl py-3 font-bold text-sm flex items-center justify-center gap-2"
              style={{
                background: bankSaved ? "#00E5A0" : (bankName && acctNum && acctName) ? "var(--cyan)" : "var(--s3)",
                color: (bankName && acctNum && acctName) ? "#060A12" : "var(--txt3)",
                cursor: (bankName && acctNum && acctName) ? "pointer" : "default",
                border: "none",
              }}
            >
